...
class SequenceReplayBuffer:
    ...
    def push(self, state, action, reward, next_state, done):
        ...
        assert state.shape == (72,), f"State shape error: {state.shape}, expected (72,)"
        assert next_state.shape == (72,), f"Next state shape error: {next_state.shape}, expected (72,)"
    ...
    def sample_sequence(self, batch_size: int, seq_len: int, device: torch.device) -> Dict[str, torch.Tensor]:
        ...
        batch_indices = []
        max_tries = 100
        while len(batch_indices) < batch_size and max_tries > 0:
            start_idx = np.random.randint(0, max(1, valid_start))
            if not np.any(self.done_buf[start_idx : start_idx + seq_len - 1]):
                batch_indices.append(start_idx)
            max_tries -= 1
        # 如果实在凑不够不跨越边界的序列，就允许少量跨越，总比程序卡死强
...
class PhysicsDroneWrapper(gym.Wrapper):
    ...
    def __init__(self, env):
        ...
        # 新增：历史动作队列 (15步 * 4维 = 60)
        self.history_steps = 15
        self.action_dim = 4
        self.action_deque = deque(maxlen=self.history_steps)
        ...
        # 更新状态空间定义为 72 维
        self.state_dim = 12 + (self.history_steps * self.action_dim)
    ...
    def reset(self, **kwargs):
        ...
        for _ in range(self.history_steps):
            self.action_deque.append(np.zeros(self.action_dim))
        processed_obs = self._get_combined_obs(obs)
        # 🔴 断言1：环境输出应该始终是 72 维
    ...
    def step(self, action):
        ...
        self.last_obs = obs
        # 2. 将本次动作存入历史（注意：通常存的是送入环境前的 raw_action）
        self.action_deque.append(action)
        combined_obs = self._get_combined_obs(obs)

        # 🔴 断言1：环境输出应该始终是 72 维
        assert combined_obs.shape == (self.state_dim,), f"Env.step returned wrong obs {combined_obs.shape}"

        self.debug_info['processed_obs_for_rl'] = combined_obs.copy()
    ...
    def _get_combined_obs(self, obs):
        # 归一化运动学状态 (12维)
        kin_norm = (obs.reshape(-1)[:12] - self.obs_mean) / self.obs_std
        # 展平历史动作 (60维)
        history = np.array(self.action_deque).flatten()
        return np.concatenate([kin_norm, history]).astype(np.float32)
...
class TD3LatentUAV:
    ...
    def train_step(self, replay_buffer: SequenceReplayBuffer, batch_size: int):
        ...
        if self.config.enable_burn_in:
            ...
            info = {"critic_loss": critic_loss.item(), "action_mean": action_mean, "action_std": action_std}
            ...
            try:
                ...
                info['q_mean'] = info_q_mean
                info['q_std'] = info_q_std
            except Exception:
            ...
            if self.total_it % self.config.policy_delay == 0:
                ...
                try:
                    ...
                    info['avg_thrust'] = float(thrust_mag.mean().item())
                except Exception as e:
...
def evaluate(env: gym.Env, agent: TD3LatentUAV, episodes: int = 10, noise: float = 0.0):
    ...
    for _ in range(episodes):
        ...
        while not done:
            ...
            # 🔴 断言1：环境输出应该保持 72 维
            assert state.shape == (72,), f"Env returned wrong obs {state.shape}"
